...
use crate::{
    ...
    type_info::DataType,
    types::Type,
    Rqlite, /*RqliteArgumentValue,*/ RqliteTypeInfo, RqliteValueRef,
};
...
impl<Tz: TimeZone> Type<Rqlite> for DateTime<Tz> {
    fn type_info() -> RqliteTypeInfo {
        RqliteTypeInfo(DataType::Datetime)
    }

    fn compatible(ty: &RqliteTypeInfo) -> bool {
        <NaiveDateTime as Type<Rqlite>>::compatible(ty)
    }
}

impl Type<Rqlite> for NaiveDateTime {
    fn type_info() -> RqliteTypeInfo {
        RqliteTypeInfo(DataType::Datetime)
    }
...
impl Type<Rqlite> for NaiveDate {
    fn type_info() -> RqliteTypeInfo {
        RqliteTypeInfo(DataType::Date)
    }
...
impl Type<Rqlite> for NaiveTime {
    fn type_info() -> RqliteTypeInfo {
        RqliteTypeInfo(DataType::Time)
    }
...
impl<Tz: TimeZone> Encode<'_, Rqlite> for DateTime<Tz>
where
    Tz::Offset: Display,
{
    fn encode_by_ref(&self, buf: &mut Vec<rqlite::Value>) -> IsNull {
        Encode::<Rqlite>::encode(self.to_rfc3339_opts(SecondsFormat::AutoSi, false), buf)
    }
}

impl Encode<'_, Rqlite> for NaiveDateTime {
    fn encode_by_ref(&self, buf: &mut Vec<rqlite::Value>) -> IsNull {
        Encode::<Rqlite>::encode(self.format("%F %T%.f").to_string(), buf)
    }
}

impl Encode<'_, Rqlite> for NaiveDate {
    fn encode_by_ref(&self, buf: &mut Vec<rqlite::Value>) -> IsNull {
        Encode::<Rqlite>::encode(self.format("%F").to_string(), buf)
    }
}

impl Encode<'_, Rqlite> for NaiveTime {
    fn encode_by_ref(&self, buf: &mut Vec<rqlite::Value>) -> IsNull {
        Encode::<Rqlite>::encode(self.format("%T%.f").to_string(), buf)
    }
}

impl<'r> Decode<'r, Rqlite> for DateTime<Utc> {
    fn decode(value: RqliteValueRef<'r>) -> Result<Self, BoxDynError> {
        Ok(Utc.from_utc_datetime(&decode_datetime(value)?.naive_utc()))
    }
}

impl<'r> Decode<'r, Rqlite> for DateTime<Local> {
    fn decode(value: RqliteValueRef<'r>) -> Result<Self, BoxDynError> {
        Ok(Local.from_utc_datetime(&decode_datetime(value)?.naive_utc()))
    }
}

impl<'r> Decode<'r, Rqlite> for DateTime<FixedOffset> {
    fn decode(value: RqliteValueRef<'r>) -> Result<Self, BoxDynError> {
        decode_datetime(value)
    }
}

fn decode_datetime(value: RqliteValueRef<'_>) -> Result<DateTime<FixedOffset>, BoxDynError> {
...
impl<'r> Decode<'r, Rqlite> for NaiveDateTime {
    fn decode(value: RqliteValueRef<'r>) -> Result<Self, BoxDynError> {
        Ok(decode_datetime(value)?.naive_local())
    }
}

impl<'r> Decode<'r, Rqlite> for NaiveDate {
    fn decode(value: RqliteValueRef<'r>) -> Result<Self, BoxDynError> {
        Ok(NaiveDate::parse_from_str(&value.text()?, "%F")?)
    }
}

impl<'r> Decode<'r, Rqlite> for NaiveTime {
    fn decode(value: RqliteValueRef<'r>) -> Result<Self, BoxDynError> {
